...
if ($method === 'GET') {
    if ($action === 'get') {
        ...
        $s = $conn->prepare("SELECT usr_id, usr_username, usr_nama, usr_email, usr_status, vendor_id 
...
if ($method === 'POST') {
    ...
    if ($action === 'create') {
        ...
        if ($username === '' || $nama === '' || $email === '' || $pass === '') {
            echo "Data belum lengkap!";
            exit;
        }

        // cek duplikat
        $c = $conn->prepare("SELECT usr_id FROM tbl_user WHERE usr_username = ? OR usr_email = ? LIMIT 1");
        $c->bind_param("ss", $username, $email);
        $c->execute();
        ...
        $i = $conn->prepare("INSERT INTO tbl_user (usr_username, usr_pass, usr_nama, usr_email, usr_status, vendor_id)
                             VALUES (?, ?, ?, ?, ?, ?)");
        $i->bind_param("sssssi", $username, $hash, $nama, $email, $status, $vendor_id);
        if ($i->execute())
            echo "OK";
        else
            echo "Gagal tambah user: " . $conn->error;
        exit;
    }
    ...
    if ($action === 'edit') {
        ...
        $u = $conn->prepare("UPDATE tbl_user 
                             SET usr_nama=?, usr_email=?, usr_status=?, vendor_id=? 
                             WHERE usr_id=?");
        $u->bind_param("sssii", $nama, $email, $status, $vendor_id, $usr_id);
        if ($u->execute())
            echo "OK";
        else
